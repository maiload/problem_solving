...
> log 계산법
> - 이분탐색, 분할정복, 트리 구조 등은 **범위를 절반씩 줄이는 과정**이 반복되기 때문에, </br>
알고리즘에서 `log N`은 관습적으로 **log₂ N**을 의미 </br>
...
> </br>
> -> 높이(h)가 parameter 이며 이 값의 최소, 최대가 l 과 r이 된다
...
```
> Binary Search와 달리 주어진 배열에서가 아닌, 내가 설정한 parameter의 범위에서 최적값을 찾기 때문에 </br>
> arr[m] 과 비교하는 것이 아닌 별도의 check 메서드를 만들어 검사한다
...
## 트리 (Tree)
- 특징
  - 그래프의 일종으로 계층적인 자료 구조
  - 부모 - 자식 관계를 갖는 노드들의 집합
  - 최상위 노드가 루트
  - 사이클이 없는 비순환적인 구조
  - 모두가 **연결**되어 있는 그래프: 어떤 두 점을 골라도 간선을 타고 사이를 이동 가능
  - 정점 개수(V) = 간선 개수(E) + 1
- 용어
...
- 구현
  - bfs, dfs 모두 가능하지만, 트리는 재귀적 구조라 주로 dfs를 추천
  - 트리는 간선이 매우 적은 그래프이기 때문에 인접 리스트로 구현현
...
- **BFS**: 
  ...
  - 특징
    ...
    - visited 배열을 거리 배열로 사용하면 `방문 여부`와 `탐색 레벨`을 동시에 관리할 수 있다 -> 최단 거리 문제
      - 시작점을 구분하기 위해 초기값을 -1로 설정 : `Arrays.fill(visited, -1);`
- **키워드**
  - 공통 : 연결, 이동, 영역, 방문, 상하좌우, 인접한 칸, 격자
  - DFS : 끝까지 확인, 전부 조사
  - BFS : 확산, 퍼짐, 동시에, 최소, 가장 빠르게
 
- **공통**
  - dfs(..), bfs(..) 의 인자는 다음으로 탐색할 정점(상태)이고, </br>
    visited는 그 정점(상태)를 식별하는 최소 정보로 구성한다
  - 그래프의 구조가 변할 때
    - 그래프 탐색의 이동 과정에서 구조가 변경 -> 상태(state)로 취급
    - 구조가 변경되어야 그래프 탐색이 가능 -> 다른 알고리즘으로 그래프 구조를 먼저 결정
  - 실제 그래프문제에서는 대부분 인접 리스트 방식 O(V + E)을 사용
    - E(최대값): 한 정점에서의 이동 가능한 방향 x 정점의 개수
      - 예를 들어 4가지 방향이라면 전체 시간 복잡도: O(V + 4V) = O(V)
    - 인접 행렬: 한 정점에서 모든 정점으로의 연결 가능성을 표현
    - 인접 리스트: 한 정점에서 실제로 갈 수 있는 정점들만 표현
- `코드 예시`
  - **DFS (recursion)**
    ```
